...
!                                            T700,   TH700   
!
      REAL, ALLOCATABLE :: AFEL(:,:,:), RICHNO(:,:,:), PBLRI(:,:), PBLREGIME(:,:)
!
...
! arrays so these 2 if branches are excuted for NMMB only.
      ELSEIF(MODELNAME == 'NMM' .and. GRIDTYPE=='B' .and. imp_physics==99)THEN !NMMB+Zhao
        DO L=1,LM
          DO J=JSTA,JEND
            DO I=ista,iend
              LLMH = NINT(LMH(I,J))
          ...
          ENDDO  ! END DO J LOOP
        ENDDO    ! END DO L LOOP
      ELSEIF(MODELNAME == 'NMM' .and. GRIDTYPE=='B' .and. imp_physics==6)THEN !NMMB+WSM6
        DO L=1,LM
          DO J=JSTA,JEND
            DO I=ista,iend
              LLMH = NINT(LMH(I,J))
     ...
     &            DBZI(I,J,L) = 10.0*LOG10(DBZI(I,J,L)) ! DBZ
      ...
      ELSEIF(((MODELNAME == 'NMM' .and. GRIDTYPE=='B') .OR. MODELNAME == 'FV3R') &
...
! dong handle missing value
       ...
       ENDIF
!     
!     ASYMPTOTIC AND FREE ATMOSPHERE MASTER LENGTH SCALE (AFEL), PLUS
!     GRADIENT RICHARDSON NUMBER.
!
      IF ( (IGET(111)>0) .OR. (IGET(146)>0) .OR.         &
           (IGET(147)>0) ) THEN
        ALLOCATE(afel(IM,JSTA_2L:JEND_2U,LM))
!     
...
!             CALL MIXLEN(EL0,EL)  
            ELSE IF(MODELNAME == 'NMM')THEN
              DO L=1,LM
                DO J=JSTA,JEND
                  DO I=ista,iend
                    AFEL(I,J,L)=EL_PBL(I,J,L)  !NOW EL COMES OUT OF WRF NMM
                  ENDDO
                ENDDO
              ENDDO
            ENDIF
...
!        if(me == 0) print*,'sending input to GTG i,j,hgt,gust',i,j,ZINT(i,j,LP1),gust(i,j)
        ...
        if (.not. allocated(afel)) ALLOCATE(afel(IM,JSTA_2L:JEND_2U,LM))
        AFEL = SPVAL
        if (.not. allocated(richno)) ALLOCATE(RICHNO(IM,JSTA_2L:JEND_2U,LM))
        RICHNO = SPVAL
        ...
        call gtg_algo(im,jm,lm,jsta,jend,jsta_2L,jend_2U,                      &
                      ...
                      z0,gdlat,gdlon,dx,dy,u10,v10,GUST,avgprec,sm,sice,catedr,&
                      mwt,AFEL,gtg,RICHNO,item)
...
!	end do  
      ...
      if (allocated(pblri))  DEALLOCATE(pblri)
      if (allocated(afel))   DEALLOCATE(afel)
      if (allocated(richno)) DEALLOCATE(RICHNO)
